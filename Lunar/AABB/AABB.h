	...
	union TriangleDataFormat// VBO structure -> x, y, z, tx, ty, nx, ny, nz
	{
		...
		float& operator[](int index)
		{
			if (index >= 0 && index < 8) return d[index];
				...
				throw std::out_of_range("TriangleDataFormat: index out of range");
		}
	...
	private:
		...
		SplitEvaluationResult __FindBestSplitPlane(const AABBNode& node)
		{
			...
			for (int currAxis = 0; currAxis < 3; ++currAxis) {
				...
				for (int i = 0; i < NUM_OF_BINS - 1; ++i) {
					float planeCost = splitPlains[i].leftSurfaceArea * splitPlains[i].leftTriangleCount + splitPlains[i].rightSurfaceArea * splitPlains[i].rightTriangleCount;
		...
		void __SubdivideBFS(unsigned int parentNodeIdx)
		{
			...
			while (!Queue.empty()) {
				unsigned int currIdx = Queue.front();
				Queue.pop();
				AABBNode& node = m_Nodes[currIdx];
		...
		void AddPrimitive(const std::vector<float>& vertices, const std::vector<unsigned int>& indices)
		{
			...
			for (size_t i = 0; i < newNumOfTriangles; ++i) {
				...
				for (int j = 0; j < 3; ++j) {// for each vertex
					primitive[j] = {
							// Vertex Position     // Texture Coord // Normal Coord --> not used
							vertices[indices[i * 3 + j] * STRIDE + 0],
							vertices[indices[i * 3 + j] * STRIDE + 1],
							vertices[indices[i * 3 + j] * STRIDE + 2],
							vertices[indices[i * 3 + j] * STRIDE + 3],
							vertices[indices[i * 3 + j] * STRIDE + 4],
							vertices[indices[i * 3 + j] * STRIDE + 5],
							vertices[indices[i * 3 + j] * STRIDE + 6],
							vertices[indices[i * 3 + j] * STRIDE + 7],
					};
				}
...
//            hit.uv = glm::vec2(triangle.v0.GetUV() * w0 + triangle.v1.GetUV() * w1 + triangle.v2.GetUV() * w2);
	...
	private:
		...
		void __GenerateDebugMeshParallel(unsigned int node_idx)
		{
			...
			while (!Queue.empty()) {
				GenMeshFormat& curr = Queue.front();
				Queue.pop();
